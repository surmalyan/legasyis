    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 pb-28">
        <div className="w-full max-w-md">
          ...
          {/* Question card — large, airy */}
          <div
            className={`bg-card rounded-3xl px-8 py-12 shadow-sm border border-border mb-4 transition-all duration-250 ${
              isSwapping ? "opacity-0 scale-95" : "opacity-100 scale-100"
            }`}
          >
            <p className="text-[1.65rem] leading-[1.45] font-light text-foreground text-center">
              {question}
            </p>
          </div>
          ...
          <div className="flex justify-center mb-10">
            <button
              ...
              className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors py-2 px-4 rounded-xl"
            >
          ...
          <div className="space-y-3">
            <button
              onClick={() => navigate("/entry", { state: { question, mode: "text" } })}
