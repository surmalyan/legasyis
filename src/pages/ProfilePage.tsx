...
const ProfilePage = () => {
  ...
  const steps = [
      ...
      fields: [
        {
          ...
          label: lang === "ru" ? "Полное имя" : "Full Name",
          placeholder: lang === "ru" ? "Как вас зовут?" : "What is your name?",
          type: "text",
        },
        {
        ...
        },
        {
          ...
          placeholder: lang === "ru" ? "Где вы живёте?" : "Where do you live?",
          type: "text",
        },
      ],
    },
      ...
      fields: [
        {
          ...
          label: lang === "ru" ? "Род деятельности" : "Occupation",
          placeholder: lang === "ru" ? "Чем вы занимаетесь?" : "What do you do?",
          type: "text",
        },
        {
          key: "family" as const,
          label: lang === "ru" ? "Семья" : "Family",
          placeholder: lang === "ru" ? "Расскажите о своей семье" : "Tell about your family",
          type: "textarea",
        },
        {
          key: "hobbies" as const,
          label: lang === "ru" ? "Увлечения" : "Hobbies",
          placeholder: lang === "ru" ? "Что вас вдохновляет?" : "What inspires you?",
          type: "textarea",
        },
      ],
    },
    {
      ...
      fields: [
        {
          key: "life_motto" as const,
          label: lang === "ru" ? "Жизненное кредо" : "Life Motto",
          placeholder: lang === "ru" ? "Ваш девиз или принцип жизни" : "Your motto or life principle",
          type: "textarea",
        },
        {
          key: "biggest_dream" as const,
          label: lang === "ru" ? "Главная мечта" : "Biggest Dream",
          placeholder: lang === "ru" ? "О чём вы мечтаете больше всего?" : "What do you dream about most?",
          type: "textarea",
        },
        {
          key: "grateful_for" as const,
          label: lang === "ru" ? "За что благодарны" : "Grateful For",
          placeholder: lang === "ru" ? "За что вы благодарны в жизни?" : "What are you grateful for in life?",
          type: "textarea",
        },
      ],
    },
    {
      ...
      fields: [
        {
          key: "advice_to_descendants" as const,
          label: lang === "ru" ? "Совет потомкам" : "Advice to Descendants",
          placeholder: lang === "ru" ? "Что бы вы хотели передать будущим поколениям?" : "What would you like to pass on to future generations?",
          type: "textarea",
        },
        {
          key: "would_change" as const,
          label: lang === "ru" ? "Что бы изменили" : "What Would You Change",
          placeholder: lang === "ru" ? "Если бы могли изменить одну вещь в жизни, что бы это было?" : "If you could change one thing in life, what would it be?",
          type: "textarea",
        },
      ],
    },
  ...
  const handleSave = async (nextStep?: number) => {
    ...
    try {
      ...
      setProfile((prev) => ({
        ...prev,
        completion_step: Math.max(prev.completion_step, newStep),
      }));
      ...
      <main className="flex-1 px-6 pb-36 max-w-md mx-auto w-full">
        ...
        <div className="flex gap-3 mt-8">
          {step > 0 && (
            <button
              onClick={handleBack}
              className="flex-1 flex items-center justify-center gap-2 bg-muted text-muted-foreground rounded-2xl py-3.5 text-sm font-medium transition-all active:scale-[0.97] hover:bg-accent"
            >
              <ChevronLeft size={18} />
              {t.back}
            </button>
          ...
          {step < steps.length - 1 ? (
            <button
              onClick={handleNext}
              disabled={saving}
              className="flex-1 flex items-center justify-center gap-2 bg-primary text-primary-foreground rounded-2xl py-3.5 text-sm font-medium transition-all active:scale-[0.97] disabled:opacity-40"
            >
            ...
            <button
              onClick={() => handleSave(steps.length)}
              disabled={saving}
              className="flex-1 flex items-center justify-center gap-2 bg-primary text-primary-foreground rounded-2xl py-3.5 text-sm font-medium transition-all active:scale-[0.97] disabled:opacity-40"
            >
              {saving ? <Loader2 size={18} className="animate-spin" /> : (
